...
class YOLO(object):
    ...
    def append_csv(self, path, label, x, y, z):
    ...
    def detect_image(self, yolo, image, depth, color_intrinsics, file_path):
        start = timer()
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            # int x,y to fit to get_distance
            # get distance, or we can say, z
            point_0 = int((right + left) / 2)
            point_1 = int((top + bottom) / 2)
            point_2 = rs.depth_frame(depth).get_distance(point_0, point_1)
            ...
            yolo.append_csv(file_path, label, position[0], position[1], position[2])
            # yolo.read_csv(file_path)
...
def detect_video(yolo, video_path, output_path=""):
    ...
    fps = "FPS: ??"
    prev_time = timer()
    file_path = 'obj_data\\output.csv'
    yolo.create_csv(file_path)
    while True:
        ...
        return_value, color = vid.read()
        image = Image.fromarray(color)
        image = yolo.detect_image(yolo, image, depth_frame, color_intrinsics, file_path)
